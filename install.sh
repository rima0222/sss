#!/bin/bash
# Clean Installation & Setup Script for SSH-Pro Custom Web Panel
set -e
...
def add_user(username, password, duration_seconds, volume_gb):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
        INSERT INTO users (username, password, remaining_time, total_volume, status)
        VALUES (?, ?, ?, ?, 'active')
...
def update_user(username, password, duration_seconds, volume_gb, status):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        ...
        cursor.execute("""
        UPDATE users 
        SET password=?, remaining_time=?, total_volume=?, status=?
        WHERE username=?
...
def export_backup_json():
    ...
    cursor.execute("SELECT username, password, remaining_time, total_volume, used_download, used_upload, status FROM users")
    ...
    try:
        ...
        if "users" in data:
            for u in data["users"]:
                ...
                (username, password, remaining_time, total_volume, used_download, used_upload, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
...
def get_online_users():
    online_list = []
    try:
        ps_output = subprocess.check_output("ps -eo user,pid,cmd | grep sshd", shell=True, text=True)
        for line in ps_output.splitlines():
            parts = line.split()
            if len(parts) >= 3:
                username = parts[0]
                if username not in ["root", "sshd"] and username not in online_list:
...
def create_user_api():
    data = request.json
    success, msg = add_user(data['username'], data['password'], data['remaining_time'], data['total_volume'])
...
def update_user_api():
    data = request.json
    success, msg = update_user(data['username'], data['password'], data['remaining_time'], data['total_volume'], data['status'])
    return jsonify({"success": success, "msg": msg})
...
# ۱۳. طراحی داشبورد مدیریت با کنترلر پورت‌ها و زمان باقی‌مانده (templates/index.html)
    ...
    <div class="container">
        ...
        <div class="card" style="margin-top: 20px;">
            ...
            <form id="addUserForm" style="display: flex; gap: 10px; flex-wrap: wrap;">
                <input type="text" id="addUsername" placeholder="نام کاربری" required style="flex: 1;">
                <input type="text" id="addPassword" placeholder="رمز عبور" required style="flex: 1;">
                <input type="number" id="addTime" placeholder="زمان باقی‌مانده (ثانیه)" required style="flex: 1;">
                <input type="number" step="0.1" id="addVolume" placeholder="حجم دانلود (GB)" required style="flex: 1;">
                ...
                <table>
                    ...
                    <tbody id="usersTableBody">
                        ...
                        <tr>
                            ...
                            <td><input type="text" value="{{ user.password }}" id="pwd-{{ user.username }}" style="width: 110px;"></td>
                            <td><input type="number" value="{{ user.remaining_time }}" id="time-{{ user.username }}" style="width: 120px;"></td>
                            <td><input type="number" step="0.1" value="{{ user.total_volume }}" id="vol-{{ user.username }}" style="width: 80px;"></td>
                            <td>{{ "%.2f"|format(user.used_download) }}</td>
                            <td>
                                <select id="status-{{ user.username }}" style="width: 120px;">
                                    <option value="active" {% if user.status == 'active' %}selected{% endif %}>فعال (Active)</option>
                                    <option value="paused" {% if user.status == 'paused' %}selected{% endif %}>توقف موقت (Paused)</option>
                                    <option value="expired" {% if user.status == 'expired' %}selected{% endif %}>منقضی (Expired)</option>
                                </select>
                            </td>
                            <td>
...
document.getElementById('addUserForm')?.addEventListener('submit', async (e) => {
    ...
    const res = await fetch('/api/user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username, password, remaining_time, total_volume })
    });
    const data = await res.json();
    alert(data.msg);
    if(data.success) location.reload();
...
async function saveUser(username) {
    ...
    const remaining_time = document.getElementById(`time-${username}`).value;
    ...
    const res = await fetch('/api/user/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username, password, remaining_time, total_volume, status })
    });
    const data = await res.json();
    alert(data.msg);
}
...
[Unit]
Description=SSH-Pro Management Admin Dashboard
...
# ۲۰. ساخت سرور سبک وب‌سوکت پروکسی (ws_server.py)
...
def handle_client(client_socket):
    try:
        req = client_socket.recv(1024).decode('utf-8', errors='ignore')
...
# ۲۱. راه‌اندازی کل سرویس‌ها روی لینوکس VPS
...
echo "=== آدرس وب پنل: http://YOUR_VPS_IP:8000     ==="
echo "==============================================="
